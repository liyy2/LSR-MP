...
class AtomsToGraphs:
    ...
    def convert(
        ...
        if self.r_edges:
            ...
            env_provider = AseEnvironmentProvider(self.radius)
            neighborhood_idx, offset = env_provider.get_environment(atoms)
            data.neighbor = neighborhood_idx
            data.cell_offsets = offset
        if self.r_energy:
        ...
        return data
        # return {'_positions': data.pos, 'energy_U0':data.energy, 
        # '_atomic_numbers': data.atomic_numbers, '_neighbors': data.neighbor,
        # '_cell': data.cell.squeeze(), '_cell_offset': data.offset}
