...
from lightnp.data.atoms_dataset import AtomsDataSubset
...
def train_test_split(
    ...
    Args:
        data (ltnp.data.AtomsData): full atomistic dataset
        num_train (float): ratio of training examples
        num_val (float): ratio of validation examples
    ...
    Returns:
        ltnp.data.AtomsDataSubset: subset with training data
        ltnp.data.AtomsDataSubset: subset with validation data
        ltnp.data.AtomsDataSubset: subset with test data

    """
    num_train = floor(ratio_train * len(data))
    num_val = floor(ratio_val * len(data))
    if split_file is not None and os.path.exists(split_file):
        ...
        folder_IFnotexist_create(split_file)
        if split_file is not None:
            np.savez(
                split_file, train_idx=train_idx, val_idx=val_idx, test_idx=test_idx
            )

    train = create_subset(data, train_idx, is_aug)
...
def create_subset(dataset, indices, is_aug=None):
    r"""
    ...
    Returns:
        ltnp.data.AtomsDataSubset: subset of input dataset
    ...
    return AtomsDataSubset(dataset, indices, is_aug)
