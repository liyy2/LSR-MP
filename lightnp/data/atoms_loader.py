...
def _collate_aseatoms(examples):
    ...
    # add key "true_num_group" 
    if "group_energy" in examples[0]:
        for i in range(len(examples)):
            examples[i]['true_n_group'] = torch.tensor([examples[i]['group_energy'].shape[0]])
...
class AtomsLoader(DataLoader):
    ...
    #         #if property_name == 'energy_U0': property_value = property_value.squeeze(dim = 1)
    #         property_value -= p0
    #     if divide_by_atoms:
    ...
    def get_mesh_size(self):
        for row in self:
            #print(row['group_rho'].shape)
            rho_shape = row['group_rho'].shape[-1]
            if rho_shape == 32000:
                return 20
            elif rho_shape == 864000:
                return 60
