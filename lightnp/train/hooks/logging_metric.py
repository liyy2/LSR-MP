...
class SklearnMetric(Metric):
    r"""
    A metric that wraps a sklearn metric.
    """
    ...
    def add_batch(self, batch, result):
        y = batch[self.target]
        #y = batch[self.target] - torch.sum(batch['group_energy'], dim=1)
        if self.model_output is None:
            yp = result
        else:
            if type(self.model_output) is list:
        ...
        self.model_output.append(yp.detach().cpu().numpy())
        self.gt.append(y.detach().cpu().numpy())

    def _aggregate(self):
        self.model_output = np.concatenate(self.model_output).flatten()
        self.gt = np.concatenate(self.gt).flatten()
        
    
    def aggregate(self):
        """Aggregate metric over all previously added batches."""
        self._aggregate()
        return self.func(self.gt, self.model_output)
...
class R2(SklearnMetric):
    def __init__(
        ...
        name = "R2" + target if name is None else name
...
class PearsonCorr(SklearnMetric):
    def __init__(
        self,
        target,
        model_output=None,
        ...
        name = "PearsonCorr" + target if name is None else name
    ...
    def aggregate(self):
        """Aggregate metric over all previously added batches."""
        self._aggregate()
        return self.func(self.gt, self.model_output)[0]

class SpearmanCorr(SklearnMetric):
    def __init__(
        self,
        target,
        model_output=None,
        ...
        name = "SpearmanCorr" + target if name is None else name
...
class MeanAbsoluteError(Metric):
    ...
    def add_batch(self, batch, result):
        ...
        # print(yp, yp.shape, y.shape)
        diff = self._get_diff(y, yp).detach()
        # print(diff)
        # print()
        self.values.append(torch.abs(diff).view(-1).detach().cpu())
        self.l1loss += (
            torch.sum(torch.abs(diff).view(-1), 0).cpu().data.numpy()
        )
