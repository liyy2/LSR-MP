...
class Logger_Lin():
    def __init__(self, out_dir,flush_secs = 60):
        '''
        std_out_interval: 120 seconds
        '''
        self.log_name = os.path.join(out_dir,"log.txt")
        self.flush_secs = min(int(flush_secs),1)
    ...
    def info(self, string):
        ...
        cur_time = int(time.time()) ## sceonds
        if cur_time-self.last_time > self.flush_secs:
            self.last_time = cur_time
            self.f.close()
            self.f = open(self.log_name, 'a')


def folder_IFnotexist_create(path):
    folder = os.path.dirname(os.path.abspath(path))
    if not os.path.exists(folder):
        print("File name is {}. The folder {} not exists. Creat successfully.".format(path,folder))
        os.makedirs(folder)
...
def load_model(model_path, map_location=None):
    ...
    Returns:
        :class:`lightnp.nn.exporter.AtomisticModel`: Loaded SchNetPack model.
...
def activate_stress_computation(model, stress="stress"):
    ...
    Args:
        model (lightnp.nn.exporter.AtomisticModel): SchNetPack model for which computation of the stress tensor
                                                    should be activated.
