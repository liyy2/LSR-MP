...
def get_model(config,mean,std,regress_forces,atomref):
    if config['model'] in ["TorchMD_Norm", "TorchMD_ET", "PaiNN"]:
        ...
        model = E2FormerLSRMP(
            ...
            neighbor_embedding=config.get('neighbor_embedding', True),
            short_cutoff_upper=config["short_cutoff_upper"],
            long_cutoff_upper=config["long_cutoff_upper"],
            mean=mean,
            std=std,
            atom_ref=atomref,
            max_z=config.get('max_z', 100),
            group_center=config.get('group_center', 'center_of_mass'),
            tf_writer=None,
            radius=config["short_cutoff_upper"]  # Use short cutoff as E2Former radius
        )
